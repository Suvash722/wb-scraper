...
def spider_find_pdfs(session, start_url):
    valid_pdfs = []
    try:
...
def scrape_and_build_csv(target_urls_input, output_path="output.csv"):
    """
    Takes a comma-separated string of URLs, scrapes them, and writes
    the extracted ST-candidate records into a CSV file.
    Returns (output_path, record_count).
    """
    target_urls = [u.strip() for u in target_urls_input.split(',') if u.strip()]
    ...
    for url in target_urls:
        pdfs = spider_find_pdfs(session, url)
        ...
        for pdf in pdfs:
            ...
            try:
                ...
                print(f"Failed on {pdf['URL']}: {e}")

    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Year", "Department", "Source_Website", "Roll_Number", "Name", "Category"])
        writer.writerows(final_extracted_data)
